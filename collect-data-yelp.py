...
restaurants_type_html = soup.find_all(class_ = "lemon--li__373c0__1r9wz border-color--default__373c0__3-ifU")
restaurant_type_list = []
for restaurant_type in restaurants_type_html:
    restaurant_type_list+= [[restaurant_type[10:-4] for restaurant_type in re.findall(r'target="">.*?</a>',str(restaurant_type.find_all(class_ ='lemon--a__373c0__IEZFH link__373c0__1G70M link-color--inherit__373c0__3dzpk link-size--default__373c0__7tls6')))]]
...
for possition in range(len(restaurant_star)):
    print(restaurant_address[possition][7:-1] + " " + restaurant_name[possition][6:-1]+ " "+ (restaurant_star[possition])[:-22] + " " + restaurant_price[possition][7:-7]+ " "+ str(restaurant_type_list[possition]))